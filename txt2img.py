...
def load_model_from_config(config, ckpt, device="cuda", verbose=False):
    ...
    sd = pl_sd["state_dict"]    
    print(f"Loading state_dict from {ckpt}")
...
def main(opt):
    ...
    if opt.torchscript:
        ...
        if opt.bf16 and not opt.torchscript :
...
#     parser = argparse.ArgumentParser("Text to image by stable diffusion", parents=[parse_args()])

#     text = "a professional photograph of supermen flying in the air"
